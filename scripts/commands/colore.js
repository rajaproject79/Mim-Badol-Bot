...
module.exports.run = async function ({ api, event, args }) {
  if (args.length < 1) {
    return api.sendMessage("Kindly furnish a designated color denomination or its corresponding hexadecimal code. This command is employed as follows: /color [color].", event.threadID, event.messageID);
  }
  ...
  if (colorMap[colorInput]) {
    ...
    outputStream.on("finish", () => {
      const response = `⌲ 𝗥𝗘𝗦𝗨𝗟𝗧\n\ndecimal code for ${colorInput} is ${hexCode}`;
      api.sendMessage(
